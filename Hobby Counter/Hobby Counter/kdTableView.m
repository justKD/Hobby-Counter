...
- (void)didMoveToSuperview {
    ...
    if (self.hasCloseButton) {
        float closeSize = 40;
        UILabel *close = [[UILabel alloc] initWithFrame:CGRectMake(VBW(self)-closeSize-20, OYB(self)+SB(), closeSize, closeSize)];
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(closeTable:)];
...
- (void)makeTableViewOnView:(UIView*)view {
    ...
    UIView *mask = [[UIView alloc] initWithFrame:CGRectMake(self.bounds.origin.x,self.bounds.origin.y+self.topMargin,self.bounds.size.width-15,self.bounds.size.height-self.bottomMargin)];
    
    tableView = [[kdSetupTableView alloc] init];
    tableView.frame = mask.bounds;
...
- (UIView*)makeDialoguePopup {
    ...
    float closeSize = 40;
    UILabel *close = [[UILabel alloc] initWithFrame:CGRectMake(VBW(view)-closeSize-20, OYB(view)+SB(), closeSize, closeSize)];
    [close setFont:[UIFont fontWithName:KDFontNormal size:20]];
    [close setUserInteractionEnabled:YES];
    [close setText:@"X"];
    [close setTextAlignment:NSTextAlignmentCenter];
